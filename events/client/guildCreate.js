...
module.exports = async (client, guild) => {
	...
	if (await db.get(`antijoinbot_${client.user.id}`) === null) {
		...
		let own = await db.all().filter(data => data.ID.startsWith(`ownermd_${client.user.id}`)).sort((a, b) => b.data - a.data)
	...
	} else if (await db.get(`antijoinbot_${client.user.id}`) === true) {
		let own = await db.all().filter(data => data.ID.startsWith(`ownermd_${client.user.id}`)).sort((a, b) => b.data - a.data)
